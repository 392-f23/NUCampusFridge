...
def get_logo_url(item):
    ...
    if 'items' not in response_json:
        print(f'No search results for {item}')
        return None
...
# Get the start index
start_index = next((index for (index, d) in enumerate(main_data) if d['Item'] == 'cinnamon raisin'), None)
